...
MEDIA_VERBS = [
    "http://lecognizer.com/xapi/profiles/media/1.0/verbs/initialized",
    ...
    "http://lecognizer.com/xapi/profiles/media/1.0/verbs/completed",
    "http://lecognizer.com/xapi/profiles/media/1.0/verbs/terminated",
    "http://lecognizer.com/xapi/profiles/media/1.0/verbs/interacted",
]

def main():
    p = argparse.ArgumentParser(description="Media verb key-flow checker (streaming)")
    ...
    p.add_argument("--profile-id", default=DEFAULT_MEDIA_PROFILE_ID)
    p.add_argument("--verb", action="append", default=[], help="분석할 verb (여러 번 지정 가능)")
    p.add_argument("--verbs", choices=["all"], help="all: 미리 정의된 media verbs 전체")
    ...
    for verb_id in verbs:
        setup_logging(log_dir=args.log_dir, verb_id=verb_id or "media")
