# navigation_verb_flow_check/io/source_iter.py
import json
import time
import logging
...
# 기본 Navigation 프로파일 ID (너희 환경에 맞춰 교체 OK)
DEFAULT_NAVIGATION_PROFILE_ID = "http://lecognizer.com/xapi/profiles/navigation"
VERB_NAVIGATED = (
...
def iter_mongo_verb(
    ...
    end_yyyymm: str,
    batch_size: int = 2000,
    verb_id: str = "",                      # 네비게이션도 verb로 필터
    profile_id: str = DEFAULT_NAVIGATION_PROFILE_ID,  # ✅ Navigation용 프로파일 ID
) -> Iterator[dict]:
    """Navigation 프로파일 + 지정 verb 문서를 월 범위로 한 번에 스트리밍(range 쿼리)."""
    ...
    query = {
        ...
        "statement.context.contextActivities.category.id": profile_id,
    }
    if verb_id:
...
def iter_mongo_verb_monthwise(
    ...
    batch_size: int = 2000,
    verb_id: str = "",
    profile_id: str = DEFAULT_NAVIGATION_PROFILE_ID,  # ✅
