...
def main():
    ...
    print("Profiling a medium-scale subset of 1,000,000 records from lrs.statements...")
    t0 = time.time()
    ...
    print(f"\nClassification Results for the 1M subset:")
    print("-" * 50)
    print(f"1. Real Students (Korean Human Names): {len(real_students)} accounts")
    print(f"2. Testing/Tester Accounts: {len(rich_testers)} accounts")
    
    print("\nTop 15 Real Students found in 1M subset:")
    for rs in sorted(real_students, key=lambda x: x["count"], reverse=True)[:15]:
